...
public interface NotificationSettingsRepository extends JpaRepository<NotificationSetting, Integer> {
    ...
    @Query("select n from NotificationSetting n where n.username=?1")
    NotificationSetting findbyUsername(String s);

    @Transactional
    @Modifying
    @Query(value = "update notification_settings set payment_reminders=?2, low_stock_alert=?3, system_updates=?4, updated_date = ?5, updated_by=?1 where username=?1", nativeQuery = true)
